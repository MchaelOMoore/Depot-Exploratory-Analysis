...
csv_path = 'C:\\Users\\Micha\\OneDrive\\Projects\\depot_incident_analysis\\csv\\contact_with_objects_incidents.csv'
...
# Define all possible categories, including 'Fatal'
injury_categories = ['Fatal', 'Non-severe', 'Severe', 'Shock/Trauma', 'Specified', 'None']

# Convert 'injuries simplified' to a categorical type with explicit categories
smis_df1['injuries simplified'] = pd.Categorical(
    smis_df1['injuries simplified'],
...
smis_counts2 = smis_df1.groupby(['period', 'injuries simplified'])['smis_reference'].count().unstack('injuries simplified').fillna(0)

# Add a column for total depot incidents
smis_counts2['TotalContactWithObjectIncidents'] = smis_counts2.sum(axis=1)
...
for i in range(Nyears+1):       #need indents or otherwise it doesn't work
    p1 = np.arange(1,14)
...
smis_data2['STF Events AMA'] = smis_data2['TotalContactWithObjectIncidents'].rolling(window=13).mean()
...
conf_intervals_mean = np.mean(conf_intervals_cut['TotalContactWithObjectIncidents']) #calculate the mean of the cut of data

bounds = st.norm.interval(confidence=0.95, loc=conf_intervals_mean, scale=st.sem(conf_intervals_cut['TotalContactWithObjectIncidents']))       #I think it is correct to use the standard error of the mean as we are looking at the whole population
...
ind = np.arange(0,N,1)
bordercolour = '0.6'
graphtitle = 'Trend in STF events' #graph title
...
nonebar = np.array(smis_data2['None'])
...
ax1.bar(ind, nonebar, width, color=(225/255, 234/255, 232/255), zorder=3, 
        bottom=fatalbar + specifiedbar + severebar + nonseverebar + shocktraumabar, label='None')


ax1.plot(ind, smis_data2['STF Events AMA'], color='black',label='Total annual moving average (AMA)')
