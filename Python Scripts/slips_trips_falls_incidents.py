...
csv_path = 'C:\\Users\\mmoor\\OneDrive\\SQL\\slips_trips_falls_incidents.csv'
...
smis_counts2['TotalSTFIncidents'] = smis_counts2.sum(axis=1)
...
smis_data2['STF Events AMA'] = smis_data2['TotalSTFIncidents'].rolling(window=13).mean()
...
conf_intervals_mean = np.mean(conf_intervals_cut['TotalSTFIncidents']) #calculate the mean of the cut of data

bounds = st.norm.interval(confidence=0.95, loc=conf_intervals_mean, scale=st.sem(conf_intervals_cut['TotalSTFIncidents']))       #I think it is correct to use the standard error of the mean as we are looking at the whole population
